...
public final class GroupLensDataModel extends FileDataModel {
	...
	private static final Pattern COLON_DELIMITER_PATTERN = Pattern
			.compile(COLON_DELIMTER);
	...
	private static File convertGLFile(File originalFile) throws IOException {
		...
		// comma
		File resultFile = new File(new File(
				System.getProperty("java.io.tmpdir")), "ratings.txt");
		...
		try {
			...
			for (String line : new FileLineIterable(originalFile, false)) {
				...
				String convertedLine = COLON_DELIMITER_PATTERN.matcher(subLine)
						.replaceAll(",");
	...
	public static File readResourceToTempFile(String resourceName)
		...
		try {
			URL resourceURL = Resources.getResource(Recommender.class,
					resourceName);
